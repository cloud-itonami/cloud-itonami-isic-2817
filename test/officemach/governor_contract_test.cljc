(ns officemach.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control assembly/test-bench equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue a safety-certification mark')
  implemented faithfully. The single invariant under test:

    OfficeMachOpsAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Office Machinery Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :calculator}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :calculator (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the safety-certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-dielectric-withstand-test-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:dielectric-withstand-test-kv 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-dielectric-withstand-test-kv} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:dielectric-withstand-test-kv (store/batch db "batch-001"))) "fabricated dielectric-withstand-test reading never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "assembly-001" :severity :moderate
                                            :description "electrical insulation degradation suspected"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "assembly-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
