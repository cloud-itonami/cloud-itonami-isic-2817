(ns officemach.advisor
  "OfficeMachOpsAdvisor -- the *contained intelligence node* for the
  office-machinery plant-operations coordination actor.

  It normalizes production-batch patches (product-type/dielectric-
  withstand-test-kv/quantity/defect-rate), drafts an assembly/test-
  bench-equipment maintenance scheduling proposal against a piece of
  equipment, drafts a safety-concern flag, and drafts an outbound
  office-machinery shipment coordination proposal against a
  ...
  a committed record and NEVER a real assembly/test-bench-equipment
  actuation, freight dispatch, or safety certification -- see README
  `What this actor does NOT do`. Every output is censored downstream
  by `officemach.governor` before anything touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct assembly/
  ...
  `officemach.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [officemach.registry :as registry]
...
(defn- log-production-batch
  ...
  dielectric-withstand-test-kv, quantity, defect-rate or verification
...
(defn- flag-safety-concern
  "Draft an electrical-safety/mechanical-safety/UL-CE-compliance
  ...
  technicality). See `officemach.phase`: no phase ever adds this op to
  a phase's `:auto` set; `officemach.governor` also always escalates
  on `:coordination/safety-concern`. Two independent layers agree,
  deliberately."
  [db {:keys [subject value]}]
  (let [equipment-id (:equipment-id value)
...
(defn- coordinate-shipment
  "Draft an outbound office-machinery shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed unit quantity -- it does NOT invent one, and
  `officemach.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは事務機器(タイプライター・電卓・レジスター・複写機等、"
       "コンピュータ及び周辺機器を除く)製造工場"
       ...
       "組立・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "安全認証(UL/CE等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `officemach.governor`
