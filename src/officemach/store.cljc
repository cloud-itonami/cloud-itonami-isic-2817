(ns officemach.store
  "SSoT for the office-machinery plant-operations coordination actor,
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2652`'s own
  `watchmfg.store`, `cloud-itonami-isic-2710`'s own
  ...
  is self-contained (no external office-machinery-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/dielectric-withstand-test-kv/
    ...
    - `equipment`         -- an assembly/test-bench unit's own record.
    ...
    - `maintenance`       -- a scheduled assembly/test-bench
                             maintenance-window DRAFT against a piece
                             of equipment (`officemach.registry`'s
    ...
    - `shipments`         -- a proposed outbound office-machinery
                             shipment DRAFT (`officemach.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (officemach.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (officemach.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :calculator
                :model "CALC-100" :dielectric-withstand-test-kv 1.5
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :photocopier
                :model "COPY-200" :dielectric-withstand-test-kv 2.2
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :cash-register
                :model "REG-15" :dielectric-withstand-test-kv 1.8
...
(defn- sample-equipment []
  {"assembly-001" {:id "assembly-001" :kind :assembly-station
                    :verified? true :registered? true
                    :last-maintenance-date "2026-05-01"}
   "test-bench-002" {:id "test-bench-002" :kind :dielectric-test-bench
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `officemach.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `officemach.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  assembly-station unit (schedulable for maintenance), one
  UNVERIFIED/unregistered dielectric-test-bench unit (blocks any
