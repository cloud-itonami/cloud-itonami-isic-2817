(ns officemach.governor
  "Office Machinery Plant Operations Governor -- the independent
  compliance layer that earns the OfficeMachOpsAdvisor the right to
  commit. The advisor has no notion of whether a piece of equipment it
  wants to schedule maintenance against has actually been inspected/
  ...
  draft-schedule) assembly/test-bench equipment, whether a proposal
  secretly tries to self-issue a safety-certification mark (an
  authority this actor never holds), whether a shipment proposal's own
  claimed quantity would blow through the batch's own logged
  production quantity, or when an act stops being a coordination
  proposal and becomes direct assembly/test-bench-equipment control,
  so this MUST be a separate system able to *reject* a proposal and
  fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:office-machinery-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:assembly/actuate`
                                       or `:test-bench/run`) is the
                                       'direct assembly/test-bench-
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       assembly/test-bench equipment is
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       safety-certification mark (e.g.
                                       UL listing or CE marking under
                                       applicable electrical-safety/
                                       machinery directives) -- an
                                       authority exclusively reserved
                                       to the accredited certification
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`officemach.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid dielectric-
       withstand-test reading      -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:dielectric-withstand-test-kv`
                                       that is not a physically
                                       plausible electrical-safety test
                                       reading
                                       (`officemach.registry/
                                       dielectric-withstand-test-kv-
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [officemach.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct assembly/
  test-bench-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct assembly/test-bench-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は組立・試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate assembly/test-bench equipment -- this actor may
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "組立・試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a safety-certification mark (e.g. UL listing or CE
  marking under applicable electrical-safety/machinery directives) --
  an authority exclusively reserved to the accredited certification
  body, never this actor. No phase and no human approval can ever
  override this."
  [proposal]
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "安全認証(UL/CE等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-dielectric-withstand-test-violations
  "For `:log-production-batch`, if the patch declares a
  `:dielectric-withstand-test-kv` that is not a physically plausible
  electrical-safety test reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [kv (:dielectric-withstand-test-kv (:value proposal))]
      (when (and (some? kv) (not (registry/dielectric-withstand-test-kv-valid? kv)))
        [{:rule :invalid-dielectric-withstand-test-kv
          :detail (str kv "kV は物理的に妥当な絶縁耐圧試験結果の範囲外")}]))))
...
(defn check
  "Censors an OfficeMachOpsAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-dielectric-withstand-test-violations request proposal)
