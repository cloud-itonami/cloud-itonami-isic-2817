(ns officemach.operation
  "OfficeMachOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (OfficeMachOpsAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Office Machinery Plant Operations Governor (:govern) and the rollout
...
(defn build
  "Compiles an OfficeMachOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `officemach.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; OfficeMachOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Office Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern
